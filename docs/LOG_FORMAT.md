...
| `rate_hz` | `50` | Data-row rate |
| `start_utc` | `2026-09-24T10:15:02Z` | UTC of the first GNSS fix, if known when the file is created (may be written later as `# start_utc=` in a trailer) |
| `declination_deg` | `-1.2` | Magnetic declination. `yaw` is already corrected to true when this is set |
| `imu_mount` | `x_fwd_y_stbd_z_down` | Sensor-to-body mapping applied on board |
...
Unknown keys are ignored. A trailer of `#` lines at the end of the file is allowed.
...
| `yaw` | deg | Heading 0–360, true if `declination_deg` is set, else magnetic |
| `roll` | deg | Heel: **+ = starboard side down** |
| `pitch` | deg | **+ = bow up** |
| `gx`, `gy`, `gz` | deg/s | Body rates about x (forward), y (starboard), z (down) |
| `ax`, `ay`, `az` | m/s² | Body acceleration including gravity (at rest level: az ≈ −9.81 in this z-down frame, reported as measured) |
...
120020,1790158502020,1,50.8123456,-1.3123456,1.132,41.2,0.05,1.1,3,17,43.9,11.4,0.8,0.12,-0.40,1.35,0.20,1.61,-9.62,1512,1160,,,5.02,0,,
120040,1790158502040,0,50.8123456,-1.3123456,1.132,41.2,0.05,1.1,3,17,44.0,11.5,0.8,0.10,-0.35,1.30,0.18,1.63,-9.61,1512,1160,,,5.02,0,,
```
